...
async function initDB() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS players (
      ...
      last_login TEXT DEFAULT (to_char(NOW() AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"Z"'))
    );
    ...
    CREATE TABLE IF NOT EXISTS trial_results (
      id SERIAL PRIMARY KEY, player_id TEXT NOT NULL,
    ...
    CREATE TABLE IF NOT EXISTS advisor_sessions (
      token TEXT PRIMARY KEY, created_at TEXT DEFAULT (to_char(NOW() AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"Z"'))
    );
    CREATE TABLE IF NOT EXISTS push_subscriptions (
      player_id TEXT NOT NULL, subscription TEXT NOT NULL,
      created_at TEXT DEFAULT (to_char(NOW() AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"Z"'))
    );
    CREATE TABLE IF NOT EXISTS referral_clicks (
      id SERIAL PRIMARY KEY, ref_code TEXT NOT NULL, visitor_ip TEXT,
...
// Helpers wrapping pg pool
async function dbGet(sql, params=[]) {
  // Convert ? placeholders to $1, $2, etc.
  const pgSql = sql.replace(/\?/g, (_, i) => {
    // Count occurrences to assign correct index
...
// ── AUTH MIDDLEWARE ───────────────────────────────────────────────────────────
async function advisorAuth(req, res, next) {
  const token = req.headers['x-advisor-token'] || req.query.token;
  if (!token) return res.status(401).json({ error:'Unauthorized' });
  const session = await dbGet('SELECT * FROM advisor_sessions WHERE token = ?', [token]);
...
app.post('/api/start', async (req, res) => {
  try {
    ...
    await dbRun(`INSERT INTO players (id,name,phone,pet_name,archetype,what_matters,referral_code,referred_by,created_at,last_login)
           VALUES (?,?,?,?,?,?,?,?,?,?)`,
      [id, name, phone, petName, archetype, whatMatters||'Not sure', refCode, referredBy||null, now, now]);
    await dbRun(`INSERT INTO game_state (player_id) VALUES (?)`, [id]);
...
app.get('/api/player/:id', async (req, res) => {
  try {
    const player = await dbGet('SELECT * FROM players WHERE id = ?', [req.params.id]);
    if (!player) return res.status(404).json({ error:'Not found' });
    const gs = await dbGet('SELECT * FROM game_state WHERE player_id = ?', [req.params.id]);
    const trials = await dbAll('SELECT * FROM trial_results WHERE player_id = ? ORDER BY created_at DESC LIMIT 20', [req.params.id]);
    const chapters = await dbAll('SELECT * FROM chapter_progress WHERE player_id = ? ORDER BY chapter_id', [req.params.id]);
    await dbRun('UPDATE players SET last_login = ? WHERE id = ?', [new Date().toISOString(), req.params.id]);
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    const player = await dbGet('SELECT * FROM players WHERE id = ?', [playerId]);
    if (!player) return res.status(404).json({ error:'Not found' });
    const gs = await dbGet('SELECT * FROM game_state WHERE player_id = ?', [playerId]);
    ...
    await dbRun('UPDATE game_state SET login_streak = ? WHERE player_id = ?', [streak, playerId]);
    await dbRun('UPDATE players SET last_login = ? WHERE id = ?', [new Date().toISOString(), playerId]);
    const updatedGs = await dbGet('SELECT * FROM game_state WHERE player_id = ?', [playerId]);
...
app.post('/api/trial', async (req, res) => {
  try {
    ...
    await dbRun(`UPDATE game_state SET ${dimKey}=?,xp=?,guardian_level=?,trials_complete=?,scenarios_done=?,
    ...
    await dbRun(`INSERT INTO trial_results (player_id,scenario_id,beasts_used,outcome,dimension_affected,decision_time_ms,xp_gained,reward_roll)
           VALUES (?,?,?,?,?,?,?,?)`,
...
// ── CHAPTER SYSTEM ──────────────────────────────────────────────────────────
app.post('/api/game-start/:playerId', async (req, res) => {
  try {
    const { playerId } = req.params;
    for (let i = 1; i <= 5; i++) {
      const existing = await dbGet('SELECT * FROM chapter_progress WHERE player_id=? AND chapter_id=?', [playerId, i]);
      if (!existing) {
        await dbRun('INSERT INTO chapter_progress (player_id,chapter_id,status,boss_hp) VALUES (?,?,?,?)',
...
app.get('/api/chapters/:playerId', async (req, res) => {
  try {
    const rows = await dbAll('SELECT * FROM chapter_progress WHERE player_id=? ORDER BY chapter_id', [req.params.playerId]);
...
app.post('/api/chapter-trial', async (req, res) => {
  try {
    ...
    const cp = await dbGet('SELECT * FROM chapter_progress WHERE player_id=? AND chapter_id=?', [playerId, chapterId]);
    ...
    if (correct) {
      ...
      await dbRun(`UPDATE game_state SET ${dimKey}=?, xp=xp+50, trials_complete=trials_complete+1 WHERE player_id=?`, [newVal, playerId]);
    } else {
      heartsRemaining = Math.max(0, heartsRemaining - 1);
      const newVal = Math.max(5, (gs[dimKey]||40) - 10);
      await dbRun(`UPDATE game_state SET ${dimKey}=? WHERE player_id=?`, [newVal, playerId]);
    }

    await dbRun('UPDATE chapter_progress SET boss_hp=?,hearts_remaining=?,scenarios_done=? WHERE player_id=? AND chapter_id=?',
      [bossHp, heartsRemaining, JSON.stringify(done), playerId, chapterId]);

    const updatedGs = await dbGet('SELECT * FROM game_state WHERE player_id=?', [playerId]);
    const profileType = classifyProfile(updatedGs);
    await dbRun('UPDATE game_state SET profile_type=? WHERE player_id=?', [profileType, playerId]);
...
app.post('/api/chapter-complete', async (req, res) => {
  try {
    ...
    const now = new Date().toISOString();
    await dbRun('UPDATE chapter_progress SET status=?,completed_at=?,dodge_count=?,wrong_answers_json=?,age_reached=? WHERE player_id=? AND chapter_id=?',
      [won?'cleared':'failed', now, dodgeCount||0, wrongAnswers||'[]', ageReached||22, playerId, chapterId]);
    if (won && chapterId < 5) {
      const next = await dbGet('SELECT * FROM chapter_progress WHERE player_id=? AND chapter_id=?', [playerId, chapterId+1]);
      if (next && next.status === 'locked') {
        await dbRun('UPDATE chapter_progress SET status=? WHERE player_id=? AND chapter_id=?', ['active', playerId, chapterId+1]);
      }
    }
    const player = await dbGet('SELECT * FROM players WHERE id=?', [playerId]);
    const gs = await dbGet('SELECT * FROM game_state WHERE player_id=?', [playerId]);
...
app.post('/api/game-reset/:playerId', async (req, res) => {
  try {
    const { playerId } = req.params;
    await dbRun('UPDATE game_state SET evolution_stage=1,guardian_level=1,xp=0,vitality=40,stability=40,resilience=40,bond=40,legacy=40,login_streak=1,trials_complete=0,beasts_unlocked=?,scenarios_done=?,last_trial_at=NULL,profile_type=? WHERE player_id=?',
      ['[]','[]','UNENGAGED', playerId]);
    await dbRun('DELETE FROM chapter_progress WHERE player_id=?', [playerId]);
    for (let i = 1; i <= 5; i++) {
      await dbRun('INSERT INTO chapter_progress (player_id,chapter_id,status,boss_hp) VALUES (?,?,?,?)',
        [playerId, i, i===1?'active':'locked', i===5?150:100]);
    }
...
app.get('/api/ref/:code', async (req, res) => {
  try {
    ...
    const p = await dbGet('SELECT id,name,pet_name,archetype FROM players WHERE referral_code=?',[code]);
    if (!p) return res.status(404).json({ error:'Invalid code' });
    const ip = req.headers['x-forwarded-for'] || req.socket.remoteAddress || 'unknown';
    await dbRun('INSERT INTO referral_clicks (ref_code,visitor_ip) VALUES (?,?)', [code, ip]);
    res.json(p);
  } catch(e) { console.error(e); res.status(500).json({ error:e.message }); }
});
...
app.post('/api/push/subscribe', async (req, res) => {
  try {
    ...
    await dbRun('DELETE FROM push_subscriptions WHERE player_id = ?', [playerId]);
    await dbRun('INSERT INTO push_subscriptions (player_id, subscription) VALUES (?,?)',
  ...
  try {
    ...
    for (const row of subs) {
      try {
        ...
        failed++;
        if (e.statusCode === 410) {
          await dbRun('DELETE FROM push_subscriptions WHERE player_id = ?', [row.player_id]);
        }
      }
    }
...
app.get('/api/referral-stats/:playerId', async (req, res) => {
  try {
    const player = await dbGet('SELECT * FROM players WHERE id=?', [req.params.playerId]);
    if (!player) return res.status(404).json({ error: 'Not found' });
    const clicks = await dbGet('SELECT COUNT(*) as cnt FROM referral_clicks WHERE ref_code=?', [player.referral_code]);
    const friends = await dbAll('SELECT p.name, p.pet_name, p.archetype, p.created_at, gs.profile_type, gs.vitality, gs.stability, gs.resilience, gs.bond, gs.legacy FROM players p LEFT JOIN game_state gs ON gs.player_id=p.id WHERE p.referred_by=?', [player.referral_code]);
    const conversions = friends.length;
    res.json({ clicks: clicks?.cnt || 0, conversions, friends });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.get('/api/friend-compare/:playerId/:friendId', async (req, res) => {
  try {
    const me = await dbGet('SELECT p.*,gs.vitality,gs.stability,gs.resilience,gs.bond,gs.legacy,gs.guardian_level,gs.trials_complete,gs.beasts_unlocked,gs.evolution_stage FROM players p LEFT JOIN game_state gs ON gs.player_id=p.id WHERE p.id=?', [req.params.playerId]);
    const friend = await dbGet('SELECT p.*,gs.vitality,gs.stability,gs.resilience,gs.bond,gs.legacy,gs.guardian_level,gs.trials_complete,gs.beasts_unlocked,gs.evolution_stage FROM players p LEFT JOIN game_state gs ON gs.player_id=p.id WHERE p.id=?', [req.params.friendId]);
...
app.post('/api/advisor/auth', async (req,res) => {
  try {
    ...
    const token = uuidv4();
    await dbRun('INSERT INTO advisor_sessions (token,created_at) VALUES (?,?)',[token,new Date().toISOString()]);
    res.json({ ok:true, token });
  } catch(e) { res.status(500).json({ error:e.message }); }
});

app.get('/api/advisor/clients', advisorAuth, async (req,res) => {
  try {
    const clients = await dbAll(`SELECT p.*,gs.evolution_stage,gs.guardian_level,gs.xp,gs.vitality,gs.stability,
...
app.get('/api/advisor/client/:id', advisorAuth, async (req,res) => {
  try {
    const player  = await dbGet('SELECT * FROM players WHERE id=?',[req.params.id]);
    if (!player) return res.status(404).json({ error:'Not found' });
    const gs      = await dbGet('SELECT * FROM game_state WHERE player_id=?',[req.params.id]);
    const trials  = await dbAll('SELECT * FROM trial_results WHERE player_id=? ORDER BY created_at',[req.params.id]);
